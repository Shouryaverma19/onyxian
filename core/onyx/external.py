...
def _peek_name(module_yaml: Path) -> str:
    try:
        ...
        raise OnyxError(f"cannot parse {module_yaml}: {exc}") from None
    name = (data or {}).get("name")
    if not isinstance(name, str) or not name:
        raise OnyxError(f"{module_yaml} has no usable 'name'")
    return name
...
def fetch_external(spec: str, scratch: Path) -> tuple[Manifest, str, str | None]:
    ...
    if "://" in spec or spec.startswith("git@") or (Path(spec) / ".git").exists():
        ...
        if not module_yaml.is_file():
            raise OnyxError(f"{spec} has no module.yaml at its root; not an Onyx module repository")
    ...
    raise OnyxError(f"{spec!r} is neither an installed module id, a git URL, nor a module directory")
